...
UPFAdminSubtractUserVirtualCurrency* UPFAdminSubtractUserVirtualCurrency::SubtractUserVirtualCurrency(UObject* WorldContextObject, class APlayerController* PlayerController , const FString& InPlayFabId, const FString& InVirtualCurrency, const int32& InAmount)
{
...
void UPFAdminSubtractUserVirtualCurrency::Activate()
{
	...
	if(AdminAPI.IsValid())
	{	
		CallResult = AdminAPI->SubtractUserVirtualCurrency(Request, SuccessDelegate, ErrorDelegate);
	}
...
void UPFAdminSubtractUserVirtualCurrency::OnErrorCallback(const PlayFab::FPlayFabError& InError)
{
	FBPAdminModifyUserVirtualCurrencyResult BPResult;
 	OnFailure.Broadcast(BPResult);	
}
